...
Additional package for the Bayesian VAR model:

```{r rpkgs2, warning = FALSE, message = FALSE, echo = TRUE}
if (!require("cmdstanr")) {
...
if (!require("bayesplot")) {
  install.packages("bayesplot", repos = repos)
}
...
```{r bonus, eval = run_models, warning = FALSE, message = FALSE, echo = TRUE}
...
```{r bonus2, warning = FALSE, message = FALSE, echo = TRUE, eval = run_models}
...
stan_diagnostics(bayesian_fit, data_list)
...
```{r cpp1}
cat(readLines("https://raw.githubusercontent.com/pachadotdev/ecod025/refs/heads/code-for-ps1/src/00-model-metrics.h"), sep = "\n")
```

## VAR model

```{r cpp2}
cat(readLines("https://raw.githubusercontent.com/pachadotdev/ecod025/refs/heads/code-for-ps1/src/01-var.h"), sep = "\n")
```

## FA-VAR model

```{r cpp3}
cat(readLines("https://raw.githubusercontent.com/pachadotdev/ecod025/refs/heads/code-for-ps1/src/02-favar.h"), sep = "\n")
```

## DFM model

```{r cpp4}
cat(readLines("https://raw.githubusercontent.com/pachadotdev/ecod025/refs/heads/code-for-ps1/src/03-dfm.h"), sep = "\n")
```
